...
def reload_pre_train_model(model, device, path=""):
    if not bool(path):
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and (k[7:12] == 'imcnn' or k[7:17] == 'bm_encoder')}
        print(len(pretrained_dict.keys()))
...
def train(train_dataloader, net, optimizer, epoch, device, logger, scheduler, opt):
    ...
    for batch_idx, (im, ddm) in enumerate(train_dataloader, 0):
        
        optimizer.zero_grad()
        im = im.to(device)
        ddm = ddm.to(device)
        
        loss,pred,mask = net(im,ddm)
