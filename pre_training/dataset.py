...
class Dataset(data.Dataset):
    def __init__(self, mode):
        self.mode = mode
        self.img_list = glob.glob('../dataset1/data/*.jpg')
        self.img_list.sort()
        self.imagenet_mean = np.array([0.485, 0.456, 0.406])
        self.imagenet_std = np.array([0.229, 0.224, 0.225])
        
        self.bm_list = glob.glob('../dataset1/ddm/*.npy')
        self.bm_list.sort()
    ...
    def __getitem__(self, index):
        ...
        # deal gt
        bm = np.load(self.bm_list[index])  # ddm ground truth 256*256*1
        lbl = torch.from_numpy(bm).permute(2, 0, 1).float()
        return im,lbl
